...
story_paths() {
	...
	*)
		echo "error: unknown story id '$1' (use 1.2, 1.3, 1.4, or 1.5)" >&2
		exit 1
		;;
	esac
}
...
ensure_workspace_green() {
	...
	while ((iter < max)); do
		...
		iter=$((iter + 1))
		local snippet
		snippet="$(tail -n 400 "${logf}" 2>/dev/null || true)"
		rm -f "${logf}"
...
Target: **Epic 1 Story ${ver}** (see epics.md section '### Story ${ver}:').
...
Scope the review to changes and code paths introduced for **Epic 1 Story ${ver}**. Use adversarial layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) and produce triaged, actionable findings.
...
if [[ ${#STORIES[@]} -eq 0 ]]; then
	...
	echo "error: specify story id(s), e.g. 1.2 or --all" >&2
	echo "Try: $0 --help" >&2
	exit 1
fi
