...
- [`internal/app/ux_journey_run.go`](../../internal/app/ux_journey_run.go) (`RunUXJourneyCapture` step `id`s; **`ui-real/`** for extended UX loop)
...
| 1.6 | Scan CLI | `1-6-scan-cli.md` | `internal/cli`, `tests/e2e/cli_test.go` (recursive, dry-run, duplicate) | `—` | PARTIAL (CLI not in journey PNGs; e2e expanded) |
| 1.7 | Import CLI | `1-7-import-cli.md` | `internal/cli`, `tests/e2e/cli_test.go` (recursive backfill) | `—` | PARTIAL |
...
| 2.7 | Delete / trash | `2-7-delete-quarantine.md` | `internal/app`, `internal/store` | `review_delete_confirm_dialog` in **`ui-real/`** journey | PARTIAL |
...
| 2.10 | Quick collection assign FR-08 | `2-10-quick-collection-assign.md` | `internal/app` | `review_quick_assign_menu` in **`ui-real/`** | PARTIAL |
...
| 3.4 | Privacy WCAG | `3-4-share-privacy-wcag.md` | `internal/share/http_test.go` (`TestShareHTTP_HTML_wcagBaseline`) | `—` | PARTIAL (automated HTML baseline; full WCAG tooling manual) |
...
| Story | FR / UX (short) | Artifact | Automated evidence | Journey capture | Loop risk |
|-------|-----------------|----------|----------------------|-----------------|-----------|
| 4.1 | FR-33 packages | `4-1-multi-asset-snapshot-packages.md` | `internal/share`, `internal/app` share flows | `review_package_share_preview` in **`ui-real/`** | PARTIAL |
...
Aligned with `steps.json` `omissions` in [`ux_journey_run.go`](../../internal/app/ux_journey_run.go): native file picker, real OS DPI, **CLI scan/import URL open**, etc. Extended **`step_ux` / `flow_ux`** judges require **`ui-real/`** captures with `"app_mode": "real_binary"`. Judges should list these under **Gaps** when verdict touches those FRs.
